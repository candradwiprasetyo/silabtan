...
<?php
	...
	if($komentar != '' ){
		...
		if($komentar != ''){ // and $commentup != ''){
			$query = mysql_query("INSERT INTO tsilabtan_status(username, idproduct, status, tsi_idstatus, datecreated, generalsellorneedorinfo) VALUES('$email','$idproduct','$komentar','$commentup','$time','$jualbeli')");
		...
		if($jualbeli = "market"){
			$query = mysql_query("INSERT INTO tsilabtan_need(kota, idstatus, idproduct, idmeasureunit,quantity) VALUES('$idcity', '$idstatus','$idproduct','$idsatuan2','$stock')");
	...
	$queryKomen = mysql_query("SELECT st.idstatus, st.username, st.status, st.attachment, st.datecreated,
       ...
       CONCAT('butuh: ', p.`NAMEOFPRODUCT`,' sebanyak: ',n.QUANTITY,' ', m.`NAMEOFMEAUNIT`) berita
		FROM tsilabtan_need n
		JOIN tsilabtan_district d ON (n.`KOTA` = d.`IDDISTRICT`)
		JOIN tsilabtan_city c ON (d.IDCITY = c.IDCITY)
		JOIN tsilabtan_status st ON (st.`IDSTATUS` = n.`IDSTATUS`)
		JOIN tsilabtan_product p ON (p.`IDPRODUCT` = n.`IDPRODUCT`)
		JOIN tsilabtan_user u ON (u.`USERNAME` = st.`USERNAME`)
		JOIN `tsilabtan_measureunit` m ON(m.`IDMEASUREUNIT` = n.`IDMEASUREUNIT`)		
          ...
          <div class='panel panel-default panel-timeline'>
            <div class='panel-heading'>
                ...
                <div class='media'>
                    ...
                    <div class='media-body'>
                        <h4 class='text-primary'>".$hasilKomen['fullname']." <small>".$status."</small><BR /></h4>
            ...
            <div class='panel-body'>
                ...
                <div class='media-body'>
                  ...
                  <p>".$hasilKomen['status']."<BR /><BR />".$hasilKomen['kota']."<BR />".$hasilKomen['berita']."</p>
                </div>
              </div>
