...
<?php
	...
	if(isset($_REQUEST['satuan2'])){
	$idsatuan2 = $_REQUEST['satuan2'];
	}else{
	$idsatuan2 = "";
	}
	...
	if(isset($_REQUEST['kuantitas'])){
	$stock = $_REQUEST['kuantitas'];
	}else{
	$stock = "";
	}
	if(isset($_REQUEST['idKomen'])){
	...
	if($komentar != '' ){
		...
		if($jualbeli = "market"){
			$query = mysql_query("INSERT INTO tsilabtan_sell(kota,idmeasureunit,idmeasureunit2,idproduct, idstatus,price,stock) VALUES('$idcity', '$idsatuan','$idsatuan2',$idproduct,'$idstatus','$price','$stock')");
		...
		//echo $time;
			...
			$query = mysql_query("INSERT INTO tsilabtan_status(username, idproduct, status, tsi_idstatus, datecreated) VALUES('$email','$idproduct','$komen','$idKomen','$time')");
			//echo "INSERT INTO commentpengguna VALUES('','".$email."','".$komentar."','".$commentup."','".$time."')";
	...
	$queryKomen = mysql_query("SELECT st.idstatus, st.username, st.status, st.attachment, st.datecreated, 
       u.fullname,u.pictureprofile, CONCAT(d.`NAMEOFDISTRICT`,',',c.`NAMEOFCITY`,', ') kota,
       p.`NAMEOFPRODUCT`, CONCAT('harga: ',s.`PRICE`, '/',m.`NAMEOFMEAUNIT`,', stok:',
       s.`STOCK`, ' ',m2.`NAMEOFMEAUNIT`,', ') berita
		FROM tsilabtan_sell s
		JOIN tsilabtan_district d ON (s.`KOTA` = d.`IDDISTRICT`)
		JOIN tsilabtan_city c ON (d.IDCITY = c.IDCITY)
		JOIN tsilabtan_status st ON (st.`IDSTATUS` = s.`IDSTATUS`)
		JOIN tsilabtan_product p ON (p.`IDPRODUCT` = s.`IDPRODUCT`)
		JOIN tsilabtan_user u ON (u.`USERNAME` = st.`USERNAME`)
		JOIN `tsilabtan_measureunit` m ON(m.`IDMEASUREUNIT` = s.`IDMEASUREUNIT`)
		JOIN `tsilabtan_measureunit` m2 ON(m2.`IDMEASUREUNIT` = s.`IDMEASUREUNIT2`)
          ...
          <div class='panel panel-default panel-timeline'>
            ...
            <div class='panel-body'>
                ...
                <div class='media-body'>
                  ...
                  <p>".$hasilKomen['status']."<BR /><BR />".$hasilKomen['NAMEOFPRODUCT'].", ".$hasilKomen['kota'].$hasilKomen['berita']."</p>
                </div>
              </div>
